...
ZEND_BEGIN_ARG_INFO(construct_args, 2)    //声明arg_construct为含有x,y的2个参数
ZEND_ARG_INFO(0, rulefile)
ZEND_ARG_INFO(0, imgfile)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(saveToFile_args, 1)    //声明arg_construct为含有x,y的2个参数
ZEND_ARG_INFO(0, filename)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO(search_args, 4)    //声明arg_construct为含有x,y的2个参数
ZEND_ARG_INFO(0, s)
...
const zend_function_entry searcher_methods[] = {
	PHP_ME(Searcher,__construct,construct_args,ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
...
void callback_embed(unsigned char* s,int start,int end,int level){
	...
	c = (void*)Z_LVAL_P(cvalue);
 	...
 	ret = search(c,str,str_len,level,callback_embed);
